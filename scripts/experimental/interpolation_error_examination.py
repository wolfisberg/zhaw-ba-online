...
def create_tfrecords_from_directory(dir):
    for data_dir in [_TRAINING_DATA_DIR, _TEST_DATA_DIR, _VALIDATION_DATA_DIR]:
        ...
        wav_files = librosa.util.find_files(
            directory=data_dir_path, ext=["wav"], recurse=False, case_sensitive=False
        )
        ...
        for file_path in wav_files:
            ...
            ref_matches = list(
                pathlib.Path(_BASE_PATH).rglob(f"*{file_name_stem}*.[pP][vV]")
            )
